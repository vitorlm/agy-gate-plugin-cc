...
test("runSetup authed + deps present → reports OK, exit 0, no install", async () => {
  const s = sink();
  let installCalls = 0;
...
test("runSetup throttled → RATE_LIMITED, never says 'not logged in'", async () => {
  ...
  assert.doesNotMatch(s.text(), /agy login/);
});

// --- agy not installed -------------------------------------------------------

test("runSetup AGY_NOT_INSTALLED → non-zero exit, install instruction", async () => {
  const s = sink();
  const code = await runSetup({
    probe: async () => ({
      ok: false,
  ...
  assert.equal(code, 1);
  assert.match(s.text(), /AGY_NOT_INSTALLED|Antigravity/i);
});

// --- deps absent then installed ---------------------------------------------

test("runSetup pre-installs deps when absent and reports it", async () => {
  const s = sink();
  let installCalls = 0;
  const code = await runSetup({
    probe: okProbe,
...
test("runSetup reports a dep install failure without crashing (non-zero exit)", async () => {
  const s = sink();
  const code = await runSetup({
    probe: okProbe,
    readAuthFile: () => true,
    ensureDeps: async () => ({
      ok: false,
      error: { code: "CLI_ERROR", message: "npm offline", remediation: "check connectivity" },
    }),
    config: gateConfigFromEnv({}),
    write: s.write,
  });
...
test("runSetup surfaces the effective stop-gate config from env", async () => {
  ...
  await runSetup({
    ...
    readAuthFile: () => true,
    ensureDeps: async () => ({ ok: true, installed: false }),
    config: gateConfigFromEnv({ AGY_GATE_STOP_REVIEW: "true", AGY_MAX_REVIEWS_PER_DAY: "25" }),
    write: s.write,
  });
  const t = s.text();
...
test("gateConfigFromEnv reads a custom review timeout from env", () => {
  assert.equal(gateConfigFromEnv({ AGY_GATE_TIMEOUT_MS: "120000" }).reviewTimeoutMs, 120_000);
});

test("gateConfigFromEnv reads enabled gate + cap from env", () => {
  const c = gateConfigFromEnv({
    AGY_GATE_STOP_REVIEW: "true",
    AGY_MAX_REVIEWS_PER_DAY: "10",
    AGY_GATE_ON_UNAVAILABLE: "block",
  });
