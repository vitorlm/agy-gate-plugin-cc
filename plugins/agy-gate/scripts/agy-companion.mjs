...
const MAX_PER_DAY = Number(process.env.AGY_MAX_REVIEWS_PER_DAY ?? 0);
const MAX_JOBS = Number(process.env.AGY_MAX_JOBS ?? 20);
...
/**
 * Foreground review (scope resolution + agy call), rendered to stdout.
  ...
  if (args.background) {
    ...
    if (!jobDeps) {
      process.stderr.write(
        `${renderError({ code: "CLI_ERROR", message: "--background needs CLAUDE_PLUGIN_DATA", remediation: "Run inside a Claude Code session." })}\n`,
      );
      return 1;
    }
 ...
 * `CLAUDE_PLUGIN_DATA` is unset (no place to install runtime deps).
 * @returns {Promise<number>}
 */
export async function setupCommand() {
  ...
  if (!DATA_DIR) {
    write(
      `${renderError({ code: "CLI_ERROR", message: "no data dir (CLAUDE_PLUGIN_DATA unset) — cannot install runtime deps", remediation: "Run /agy-gate:setup inside a Claude Code session." })}\n`,
    );
...
async function main(argv) {
  ...
  switch (subcommand) {
    ...
    default:
      process.stderr.write(
        "usage: agy-companion.mjs <review|adversarial-review|setup|status|result|cancel> [args]\n",
      );
      return 2;
  }
}
...
function noState() {
  process.stderr.write(
    `${renderError({ code: "CLI_ERROR", message: "no state dir (CLAUDE_PLUGIN_DATA unset)", remediation: "Run inside a Claude Code session." })}\n`,
  );
  return 1;
}
...
if (process.argv[1] === fileURLToPath(import.meta.url)) {
  main(process.argv.slice(2)).then(
    ...
    (err) => {
      process.stderr.write(
        `${renderError({ code: "CLI_ERROR", message: String(err?.message ?? err) })}\n`,
      );
      process.exit(1);
    },
  );
