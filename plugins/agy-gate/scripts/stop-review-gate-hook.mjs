...
const CONFIG = {
  enabled: env.AGY_GATE_STOP_REVIEW === "true",
  maxIterations: Number(env.AGY_GATE_MAX_ITER ?? 3),
  threshold: env.AGY_GATE_SEVERITY ?? "blocker",
  onUnavailable: env.AGY_GATE_ON_UNAVAILABLE ?? "allow", // "allow" | "block"
  streakLimit: Number(env.AGY_GATE_STREAK_LIMIT ?? 3),
  model: resolveModel(env.AGY_GATE_MODEL ?? null),
};
...
/**
 * §8 / OD-1: agy unavailable → never block uselessly by default; warn visibly, escalate on a streak.
...
function handleUnavailable(sessionId, state, error) {
  ...
  if (CONFIG.onUnavailable === "block") {
    return emit({
      decision: "block",
      reason: `agy unavailable (${error.code}): ${error.message}`,
    });
  }
  let msg = `⚠ TURN NOT REVIEWED: ${error.code} — ${error.message}`;
  if (streak >= CONFIG.streakLimit) {
    msg += `\n   (${streak} consecutive unreviewed turns — run /agy-gate:setup, raise quota, or disable the gate.)`;
  }
