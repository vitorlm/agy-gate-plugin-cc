---
description: Probe agy login (authed vs throttled), pre-install runtime deps, and review the stop-gate config.
argument-hint: ""
...
1. Run the companion `setup` (pre-installs the pinned runtime deps if absent, runs a real login probe, and prints the effective stop-gate config):
...
2. Relay its output verbatim, then interpret the auth line for the user:
   ...
   - **AGY_NOT_INSTALLED** — tell the user to install Antigravity CLI and add it to PATH. Do **not** do it yourself.
   - **NOT authenticated** (`AUTH_REQUIRED`) — tell the user to run `agy login` in their terminal. Do **not** run it yourself.
...
4. Use **AskUserQuestion** to confirm intent (you are guiding, not silently changing settings):
   - Whether to enable the converging stop review-gate (`stopReviewGate`). Note: with the gate on, an agy review runs when a turn ends — bounded by `maxIterations`, fails open + visible on agy unavailability.
   ...
   After the user chooses, tell them to apply the choice in the `/plugin` settings UI for `agy-gate` (these are `userConfig` fields; the plugin cannot write them on the user's behalf). Re-run `/agy-gate:setup` to confirm the new effective config.
